...
public class Decompress {
    public int MAX_SEQ_NUM = 2000;      //maximum sequence number
    public int MAX_CHA_NUM = 1 << 28;   //maximum length of a chromosome
    public int PERCENT = 10;    //the percentage of compressed sequence number uses as reference
    public int kMerLen = 14;    //the length of k-mer
    public int kMer_bit_num = 2 * kMerLen;  //bit numbers of k-mer
    public int hashTableLen = 1 << kMer_bit_num;    // length of hash table
    public int VEC_SIZE = 1 << 20;  //length for other character arrays
    public int min_rep_len = 15;    //minimum replace length, matched string length exceeds min_rep_len, saved as matched information

    public int seqNumber;
    public int secSeqNum;

    public int refCodeLen, refLowVecLen;
    public char[] refCode;
    public int[] refLoc;
    public int[] refBucket;
    public int[] refLowVecBegin;
    public int[] refLowVecLength;

    public char[] integerEncoding = new char[]{'A', 'C', 'G', 'T'};
    public List<String> seqPaths;
    public List<Integer> lineWidthVec;
    public List<String> identifierVec;
    public List<List<MatchEntry>> matchResultVec;
    ...
    public void mkResultDir(String filePath) {
    ...
    public int readFile(File pathFile) {
        if (!pathFile.exists()) {
    ...
    public void runLengthDecodingForLineWidth(BufferedReader br) {
        int length = 0;
        String str;
        try {
            str = br.readLine();
    ...
    public void getIdentifier(BufferedReader br) {
        String info;
        try {
    ...
    //构建hash表
    public void kMerHashingConstruct() {
    ...
    public void referenceSequenceExtraction(String referencePath) {
    ...
    public void readOtherData(Sequence sequence) {
        try {
    ...
    public void getMatchResult(int seqId, int pos, int length, List<MatchEntry> matchResult) {
        for (int i = 0; i < length; i++) {
            matchResult.add(matchResultVec.get(seqId).get(pos++));
        }
    }

    public void readMatchResult(Sequence sequence) {
    ...
    public void readTargetSequenceCode(Sequence sequence) {
    ...
    private void saveSequenceFile(Sequence sequence, int seqNum, String filePath) {
        File file = new File(getResultPath(filePath) + seqNum + ".fa.decompress");
        if (file.exists()) {
            file.delete();
        }
    ...
    public void Decompress(String filePath) {
        if (seqNumber > 1) {
            String lngcPath = filePath + ".lngc";    //stored the base of genomes
            String descPath = filePath + ".desc";    //stored the lineWidth and identifier
            File lngcFile = new File(lngcPath);
            if (!lngcFile.exists()) {
                throw new MyException("lngc file does not exists");
            }
            ...
            try {
                br1 = new BufferedReader(new FileReader(lngcPath));
    ...
    public void initial(File pathFile) {
        if (!pathFile.exists()) {
